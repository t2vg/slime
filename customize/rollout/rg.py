...
async def calculate_turn_reward(args: Namespace, sample: Sample, think_start_id: int, think_end_id: int, im_end_id: int, im_start_id: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for i, (s, e) in enumerate(sample_think_spans):
        ...
        if i == len(sample_think_spans) - 1 and last_turn_invalid:
            ...
            sample.metadata["output_token_mask"][s:e] = 0
            continue
        reasonable_rewards[s:e] = turn_rewards[i]
    #reassign loss mask
    ...
    for (ts, te), (rs, re) in zip(sample_think_spans, think_spans, strict=True):
        ...
        style_reward[ts:te] += torch.tensor(rm_log_probs[rs:re]) + torch.tensor(rm_entropy[rs:re])
...
async def generate(args: Namespace, sample: Sample, sampling_params: dict[str, Any], evaluation: bool = False) -> Sample:
    ...
    try:
        ...
        return sample
    token_rewards = reasonable_rewards + style_reward * args.style_reward_factor
    loss_mask = torch.tensor(sample.loss_mask, dtype=torch.float32)
    loss_mask[token_rewards == 0] = 0
    sample.loss_mask = loss_mask.tolist()
    mask_sum = loss_mask.sum().clamp(min=1)
    avg_reasonable_rewards = reasonable_rewards.sum() / mask_sum
    avg_style_reward = style_reward.sum() / mask_sum
